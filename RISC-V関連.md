...
- 32bit 仮想アドレス空間を:
  ...
  - に分割する.
- 仮想アドレス -> 1段目ページテーブル -> 2段目ページテーブル -> 物理アドレス の順に変換する.
  - 1段目ページテーブルの各エントリは 2段目ページテーブルの物理アドレスの "ページ番号" を格納する.
  - 2段目ページテーブルの各エントリは 物理アドレスの "ページ番号" を格納する.
  - ページ番号は 物理アドレスを ページサイズ (4KB) で割った (12bit 右シフトした) 値.
