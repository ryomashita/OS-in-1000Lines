...
long getchar(void) {
    // SBI function; EID #0x02 (Console getchar) を呼び出す
...
// システムコール (ecall によるトラップ)　を処理する
extern struct process *current_proc;
void handle_syscall(struct trap_frame *f) {
    switch (f->a3) { // a3 レジスタにシステムコール番号が入っている
        case SYS_PUTCHAR:
            putchar(f->a0);
            break;
        case SYS_GETCHAR:
            while (1) {
                long ch = getchar(); // getchar は入力がない場合 -1 を返す
                if (ch >= 0) {
                    f->a0 = ch;
                    break;
                }

                yield(); // 無限ループを避けるため、他のプロセスに CPU を譲る
            }
            break;
        case SYS_EXIT:
...
struct process *create_process(const void *image, size_t image_size) {
    ...
    *--sp = (uint32_t) user_entry;  // ra // user_entry 関数に復帰するように設定
    ...
    // Virtio ブロックデバイスの MMIO 領域をマッピングする
...
void kernel_main(void) {
    ...
    virtio_blk_init(); // Virtio ブロックデバイスの初期化
    // ディスク I/O のテスト
